...
def main() -> None:
    ...
    # Arbitrary policy modules for the two net slots, so several policies can
    # be debugged back to back in one battery: kind 1 runs --kind1 (default:
    # the pipeline control), kind 2 runs --kind2 (default: the deployed
    # learned policy). Any robot/ module generated by make_fast_policy works.
    ap.add_argument("--kind1", default="policy_linear_fast.py")
    ap.add_argument("--kind2", default="policy_fast.py")
    args = ap.parse_args()
    pipeline_only = args.pipeline
    ...
    linear = grab(args.kind1, "act_lin")
...
""", "")
    ...
    print(f"kinds: 0 = classical direct, 1 = {args.kind1}"
          + ("" if pipeline_only else f", 2 = {args.kind2}"))
