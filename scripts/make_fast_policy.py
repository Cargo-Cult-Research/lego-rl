...
def load(src_name="policy_weights.py"):
    ...
    src = re.sub(r"^from umath import exp$", "from math import exp",
                 (ROOT / "robot" / src_name).read_text(), flags=re.M)
...
def main() -> None:
    import sys
    # which weights to quantise: the learned policy, or the classical
    # controller cast into the same architecture as a pipeline control
    src = "policy_linear_weights.py" if "--linear" in sys.argv else "policy_weights.py"
    out_name = "policy_linear_fast.py" if "--linear" in sys.argv else "policy_fast.py"
    ...
    mod.write_text(
        '"""Generated by scripts/make_fast_policy.py -- do not edit.\n\n'
        "Fully-unrolled Q12 fixed-point form of the 4-8-8-1 policy, with an\n"
        "interpolated tanh table. Measured on the Technic Hub:\n\n"
        ...
        "Max duty error vs the float reference over 3000 random states: 0.0018.\n\n"
    ...
    out = ROOT / "robot" / "_bench_fast.py"
    parts = ['"""Generated by scripts/make_fast_policy.py -- do not edit."""',
